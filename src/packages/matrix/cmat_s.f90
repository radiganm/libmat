...
!! NOTICE:  This source file was automatically generated on 2017-06-25 03:25:54 by dox.
  ...
  contains
    ...
    subroutine add(y,x1,x2)
      ...
      type(CMatrix_s), intent(inout)        :: y
      type(CMatrix_s), intent(inout)        :: x1
      type(CMatrix_s), intent(inout)        :: x2
      type(real*4), pointer                 :: b(:)
      type(real*4), pointer                 :: a1(:)
      type(real*4), pointer                 :: a2(:)
      type(integer*8)                       :: k
      type(integer*8)                                :: N 
    ...
    subroutine axby(y,a1,x1,a2,x2)
      ...
      type(CMatrix_s), intent(inout)        :: y
      type(real*4), intent(in)                    :: a1
      type(CMatrix_s), intent(inout)        :: x1
      type(real*4), intent(in)                    :: a2
      type(CMatrix_s), intent(inout)        :: x2
      type(real*4), pointer                       :: p(:)
      type(real*4), pointer                       :: p1(:)
      type(real*4), pointer                       :: p2(:)
      type(integer*8)                                :: k
      type(integer*8)                                :: N 
      N = y%info%dims(1)*y%info%dims(2)
    ...
    subroutine c2f(x, p)
      ...
      type(CMatrix_s), intent(in)           :: x
      type(real*4), pointer, intent(inout)  :: p(:,:)
      type(integer*8)                       :: k
    ...
    subroutine pow(y, x, n)
      ...
      type(CMatrix_s), intent(inout)        :: y
      type(CMatrix_s), intent(inout)        :: x
      type(real*4), pointer                       :: a(:,:)
      type(real*4), pointer                       :: b(:,:)
      type(real*4), intent(in)                    :: n
      type(integer*8)                                :: k1
      type(integer*8)                                :: k2
      type(integer*8)                                :: k
      type(integer*8)                                :: r
    ...
    subroutine show_named2(p, sname_length, sname)
      ...
      type(real*4), pointer, intent(in)                :: p(:,:)
      integer(c_size_t), intent(in), value             :: sname_length
      type(c_ptr), intent(in), value                   :: sname
      character(kind=c_char,len=sname_length), pointer :: xname
      type(disp_settings)                              :: ds
    ...
    subroutine svd(x, s, vt)
      ...
      type(integer)                                  :: rwork
      type(integer)                                  :: info
      type(real*4), dimension(:,:), allocatable   :: work(:,:)
      type(real*4), dimension(:,:), allocatable   :: s(:,:)
      type(real*4), dimension(:,:), allocatable   :: vt(:,:)
      type(real*4), dimension(:,:), allocatable   :: u(:,:)
      type(integer)                                  :: m
      type(integer)                                  :: n
      call c_f_pointer(x%data,  p,  [x%info%dims(1),  x%info%dims(2)])
      m = x%info%dims(1)
      ...
      call sgesvd('A','A',m,n,p,m,s,u,m,vt,m,work,lwork,rwork,info)
